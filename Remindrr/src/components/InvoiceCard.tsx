...
import { getSettings, getClients, getInvoices, deleteInvoice, markInvoicePaid, sendReminderNow, syncInvoicesToServer } from '../lib/reminder-data';
...
export function openInvoicePrint(invoice: Invoice) {
  ...
  const statusClass = invoice.status === 'paid' ? 'paid' : status.label === 'OVERDUE' ? 'overdue' : status.label === 'DUE SOON' ? 'duesoon' : 'pending';
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Invoice ${invoice.invoiceNumber || invoice.id}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Helvetica Neue',Arial,sans-serif;background:#fff;color:#1e293b;padding:48px;max-width:800px;margin:0 auto}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:48px}.brand-name{font-size:28px;font-weight:800;color:#1e293b}.brand-sub{font-size:14px;color:#64748b;margin-top:2px}.invoice-meta{text-align:right}.invoice-title{font-size:36px;font-weight:800;color:#f97316;letter-spacing:-1px}.invoice-number{font-size:14px;color:#64748b;margin-top:4px}.invoice-number strong{color:#334155}.divider{border:none;border-top:2px solid #f1f5f9;margin:32px 0}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-bottom:40px}.info-block h3{font-size:11px;text-transform:uppercase;letter-spacing:1.5px;color:#94a3b8;margin-bottom:8px}.info-block p{font-size:15px;color:#334155;line-height:1.7}.info-block p strong{color:#1e293b}.status-badge{display:inline-block;padding:4px 12px;border-radius:999px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.5px}.status-paid{background:#dcfce7;color:#166534}.status-pending{background:#f1f5f9;color:#475569}.status-overdue{background:#fee2e2;color:#991b1b}.status-duesoon{background:#fef3c7;color:#92400e}.table{width:100%;border-collapse:collapse;margin-bottom:32px}.table th{background:#f8fafc;padding:12px 16px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:1px;color:#94a3b8;border-bottom:2px solid #e2e8f0}.table td{padding:16px;border-bottom:1px solid #e2e8f0;font-size:15px}.table .amount-row td{border-top:2px solid #e2e8f0;padding-top:16px}.payment-box{background:#f0fdf4;border:2px solid #bbf7d0;border-radius:12px;padding:24px;text-align:center;margin-top:32px}.payment-box h3{font-size:14px;color:#166534;margin-bottom:12px}.payment-link{font-size:14px;color:#16a34a;word-break:break-all}.footer{margin-top:48px;padding-top:24px;border-top:1px solid #e2e8f0;text-align:center;font-size:12px;color:#94a3b8}</style></head><body><div class="header"><div class="brand-block"><div class="brand-name">${settings.businessName || 'Remindrr'}</div>${settings.ownerName ? `<div class="brand-sub">${settings.ownerName}</div>` : ''}${settings.email ? `<div class="brand-sub">${settings.email}</div>` : ''}${settings.phone ? `<div class="brand-sub">${settings.phone}</div>` : ''}</div><div class="invoice-meta"><div class="invoice-title">INVOICE</div>${invoice.invoiceNumber ? `<div class="invoice-number"><strong>${invoice.invoiceNumber}</strong></div>` : ''}</div></div><div class="info-grid"><div class="info-block"><h3>Bill To</h3><p><strong>${client?.name || invoice.clientName || 'Customer'}</strong></p>${client?.email || invoice.clientEmail ? `<p>${client?.email || invoice.clientEmail}</p>` : ''}${client?.phone || invoice.clientPhone ? `<p>${client?.phone || invoice.clientPhone}</p>` : ''}</div><div class="info-block"><h3>Invoice Details</h3><p><span class="status-badge status-${statusClass}">${invoice.status === 'paid' ? 'PAID' : status.label}</span></p><p style="margin-top:8px"><strong>Created:</strong> ${created}</p><p><strong>Due:</strong> ${dueDate}</p></div></div><table class="table"><thead><tr><th>Description</th><th style="text-align:right">Amount</th></tr></thead><tbody><tr><td>${invoice.description || 'Professional services'}</td><td style="text-align:right">$${invoice.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td></tr><tr class="amount-row"><td><strong>Total Due</strong></td><td style="text-align:right"><strong>$${invoice.amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong></td></tr></tbody></table>${invoice.status !== 'paid' && invoice.paymentLink ? `<div class="payment-box"><h3>Pay Online</h3><a class="payment-link" href="${invoice.paymentLink}">${invoice.paymentLink}</a></div>` : ''}<div class="footer"><p>Generated by Remindrr &mdash; ${new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p></div></body></html>`;
    ...
    <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex-1 min-w-0">
          ...
          {/* Show reminder history for automated reminders */}
          {invoice.lastReminderSentAt && (
            <p className="text-green-600 text-xs mt-1 flex items-center gap-1">
              ✓ Auto-reminded {new Date(invoice.lastReminderSentAt).toLocaleDateString()}
              {invoice.followupCount ? ` (${invoice.followupCount} follow-up${invoice.followupCount > 1 ? 's' : ''})` : ''}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => navigate(`/invoices/${invoice.id}/edit`)}
            className="text-sm bg-slate-100 text-slate-600 font-bold px-4 py-2 rounded-lg hover:bg-slate-200 transition-colors">Edit</button>
          <button onClick={handleDelete} className="text-sm bg-red-50 text-red-600 font-bold px-3 py-2 rounded-lg hover:bg-red-100 transition-colors">
            <TrashIcon /></button>
        </div>
      </div>
    </div>
