...
import { getSettings, getClients, getInvoices, deleteInvoice, markInvoicePaid, syncInvoicesToServer } from '../lib/store';
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Helvetica Neue', Arial, sans-serif; background: #fff; color: #1e293b; padding: 48px; max-width: 800px; margin: 0 auto; }
  .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 48px; }
  .brand-name { font-size: 28px; font-weight: 800; color: #1e293b; }
  .brand-sub { font-size: 14px; color: #64748b; margin-top: 2px; }
  .invoice-meta { text-align: right; }
  .invoice-title { font-size: 36px; font-weight: 800; color: #f97316; letter-spacing: -1px; }
  ...
  .info-block h3 { font-size: 11px; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-bottom: 8px; }
  .info-block p { font-size: 15px; color: #334155; line-height: 1.7; }
  ...
  .status-duesoon { background: #fef3c7; color: #92400e; }
  .table { width: 100%; border-collapse: collapse; margin-bottom: 32px; }
  .table th { background: #f8fafc; padding: 12px 16px; text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; border-bottom: 2px solid #e2e8f0; }
  .table td { padding: 14px 16px; border-bottom: 1px solid #f1f5f9; font-size: 15px; color: #334155; }
  .table td:last-child { text-align: right; font-weight: 600; }
  .table .amount-row td { border-bottom: none; font-size: 18px; font-weight: 800; color: #1e293b; }
  .table .amount-row td:last-child { color: #f97316; }
  .payment-box { background: #f8fafc; border: 2px solid #e2e8f0; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 32px; }
  .payment-box h3 { font-size: 13px; text-transform: uppercase; letter-spacing: 1px; color: #94a3b8; margin-bottom: 12px; }
  .payment-link { color: #f97316; font-size: 15px; font-weight: 600; word-break: break-all; }
  .footer { margin-top: 48px; text-align: center; font-size: 13px; color: #94a3b8; }
...
<div class="header">
  <div>
    <div class="brand-name">${settings.businessName || 'Remindrr'}</div>
    ${settings.ownerName ? `<div class="brand-sub">${settings.ownerName}</div>` : ''}
    ${settings.email ? `<div class="brand-sub">${settings.email}</div>` : ''}
    ${settings.phone ? `<div class="brand-sub">${settings.phone}</div>` : ''}
  </div>
  <div class="invoice-meta">
    <div class="invoice-title">INVOICE</div>
    ${invoice.invoiceNumber ? `<div class="invoice-number"><strong>#${invoice.invoiceNumber}</strong></div>` : ''}
  </div>
</div>
...
<div class="info-grid">
  <div class="info-block">
    ...
    ${client?.email ? `<p>${client.email}</p>` : invoice.clientEmail ? `<p>${invoice.clientEmail}</p>` : ''}
    ${client?.phone ? `<p>${client.phone}</p>` : invoice.clientPhone ? `<p>${invoice.clientPhone}</p>` : ''}
  </div>
  <div class="info-block">
    <div class="right">
      <h3>Invoice Details</h3>
      <p><span class="status-badge status-${statusClass}">${invoice.status === 'paid' ? 'PAID' : status.label}</span></p>
      <p style="margin-top:8px"><strong>Created:</strong> ${created}</p>
...
<div class="footer">
  <p>Generated by Remindrr &mdash; ${new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
</div>
    ...
    <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm hover:shadow-md transition-all">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-slate-500 text-sm truncate">{client?.name || invoice.clientName} &middot; {invoice.description}</p>
          <p className="text-slate-400 text-xs mt-1">
            Due: {new Date(invoice.dueDate).toLocaleDateString()}
            {invoice.status !== 'paid' && (days < 0 ? ` &middot; ${Math.abs(days)}d overdue` : days === 0 ? ' &middot; Due today' : ` &middot; ${days}d left`)}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {invoice.status !== 'paid' && invoice.paymentLink && (
            <a href={invoice.paymentLink} target="_blank" rel="noreferrer"
              className="text-sm bg-green-50 text-green-700 font-bold px-4 py-2 rounded-lg hover:bg-green-100 transition-colors">Payment Link &rarr;</a>
          )}
          {invoice.status !== 'paid' && (
            <button onClick={() => { markInvoicePaid(invoice.id); syncInvoicesToServer(getInvoices()); onRefresh(); }}
              className="text-sm bg-green-500 text-white font-bold px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">Mark Paid &check;</button>
          )}
